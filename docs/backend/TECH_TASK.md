...
> **Парасольковий документ.** Опис архітектури, конвенцій, нефункціональних вимог, безпеки, real-time, DevOps, roadmap. Деталі кожної фічі (включно з технічними hints — API endpoints, DB, events, jobs) — у `docs/backend/features/{area}.md`.
...
- **Events як перший клас.** Cross-concern зв'язки — через Laravel Events (side-effects, notifications, analytics).
...
### 3.2 Архітектурний стиль — стандартний Laravel

Один Laravel-додаток зі стандартною структурою директорій:
...
2. **Service-layer pattern.** Кожна use-case = метод сервісу (`SessionService::create`, `SessionService::cancel`, ...).
3. **Domain events.** Будь-яка mutation, що може тригерити side-effects, видає Laravel Event (`SessionCreated`, `WorkoutLogUpdated`, ...). Listeners обробляють cross-concern логіку.
4. **WebSocket broadcasting** — через `ShouldBroadcast` інтерфейс на тих самих доменних подіях.

### 3.3 Функціональні області

Додаток охоплює такі бізнес-домени:

| Область | Ключові сутності |
|---|---|
| Auth & Identity | Users, RefreshTokens, OAuthIdentities, EmailVerification, PasswordReset, AuditLog |
| Files & Media | MediaFiles (upload pipeline, signed URLs) |
| Notifications | Notifications, DeviceTokens (FCM push) |
| Onboarding | OnboardingProgress |
| Clients (CRM) | Clients, ClientInvitations |
| Programs & Exercises | Programs, Exercises, ProgramExercises, ClientPrograms |
| Sessions & Calendar | Sessions, SessionParticipants, SessionSeries, CalendarIntegrations |
| Chat & Messaging | Conversations, Messages |
| Workout Tracking | WorkoutLogs, WorkoutLogExercises, WorkoutLogSets |
| Packages & Subscriptions | PackageTemplates, ClientPackages |
| Transactions | Transactions, Withdrawals |
| Progress Metrics | BodyMeasurements, PersonalRecords |
| Analytics | ProfileViewEvents, AnalyticsCache |

**Ключові зв'язки:**

- **Users** — центральна сутність; від неї залежить майже все.
- **Files** і **Notifications** — інфраструктурні, використовуються повсюдно.
- **Sessions** залежать від **Clients**, **Programs**, **Packages**.
- **Workout Tracking** залежить від **Sessions** і **Exercises**.
- **Chat** залежить лише від **Users** і **Files**.
- **Analytics** — read-only, залежить від **Transactions**, **Sessions**, **Users**.
...
Кожна модель має Policy-клас з методами per-action:
...
**Scope:** Auth · Users · Files · Notifications.
...
**Scope:** Onboarding · Clients · Programs · Exercises · Sessions (basic).
...
**Scope:** Chat · Workout Tracking · External Integrations · Sessions (extended).
...
**Scope:** Packages · Transactions · Progress · Analytics.
...
Усього **~53 укрупнені фічі**, розподілені по 16 функціональних областях. Кожна область — окремий файл у `docs/backend/features/`. Фічі **об'єднані за бізнес-значенням**: серії CRUD-операцій або тісно зв'язаних use-case'ів описуються однією фічею з кількома user stories.

| Область | Файл | Фіч | Phase | Стиль | Опис |
|---|---|:-:|:-:|:-:|---|
...
Кожна фіча в `features/{area}.md` описується за єдиним шаблоном (full або compact):
